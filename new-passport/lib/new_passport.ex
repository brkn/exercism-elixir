defmodule NewPassport do
  def get_new_passport(now, birthday, form) do
    with {:ok, entrance_time} <- enter_building(now),
         {:ok, manual} <- find_counter_information(now),
         counter <- manual.(birthday),
         {:ok, checksum} <- stamp_form(entrance_time, counter, form),
         passport_number <- get_new_passport_number(entrance_time, counter, checksum)
    do
      {:ok, passport_number}
    else
